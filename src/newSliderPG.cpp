...
void CnewSliderPG::produceTraj(trajFeatures & t, const vector<float> & vect_input) {
	...
	{       
		...
		slideDownUpCOEF(t, coef_slide1, t_tmp);
		...
		generate_halfStepFeatures(t_tmp, config_curr, hsdef);
		slideUpDownCOEF(t, coef_slide2, t_tmp);
...
void  CnewSliderPG::plot_com_zmp_along_x(ofstream & fb, const trajFeatures & t) {
	...
	for(int i=0; i < t.size-1 ; i++)
	{
		fb << (double) i*incrTime << "  " << t[i].comX << endl; 
	}
	fb << endl << endl;
	//After 3 endl, new index in gnuplot.	
	...
	for(int i=0; i < t.size-1 ; i++)
	{
		fb << (double) i*incrTime << "  " << t[i].zmpX << endl; 
	}
	fb << endl << endl;
}
...
void CnewSliderPG::plot_com_zmp_along_y(ofstream & fb, const trajFeatures & t) {
	...
	for(int i=0; i < t.size-1 ; i++)
	{
		fb << (double) i*incrTime << "  " << t[i].comY << endl; 
	}
	fb << endl << endl;
	//After 3 endl, new index in gnuplot.	
	...
	for(int i=0; i < t.size-1 ; i++)
	{
		fb << (double) i*incrTime << "  " << t[i].zmpY << endl; 
	}
	fb << endl << endl;
}
...
void CnewSliderPG::plot_feet_height(ofstream & fb, const trajFeatures & t){
	...
	for(int i=0; i < t.size-1 ; i++)
	{
		fb << (double) i*incrTime << "  " << t[i].leftfootHeight << endl; 
	}
	fb << endl << endl;	
	//After 3 endl, new index in gnuplot.	
	...
	for(int i=0; i < t.size-1 ; i++)
	{
		fb << (double) i*incrTime << "  " << t[i].rightfootHeight << endl; 
	}
	fb << endl << endl;	
}